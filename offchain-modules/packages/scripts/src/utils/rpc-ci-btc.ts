...
function generateCases(CKB_TEST_ADDRESS: string, ETH_TEST_ADDRESS: string, ETH_TOKEN_ADDRESS: string) {
  const loginCases = [
    {
      description: 'login should be successfun',
      payload: {
        ckbAddress: CKB_TEST_ADDRESS,
      },
      send: true,
    },
  ];
...
async function login(ckb: CKB, client: JSONRPCClient, CKB_PRI_KEY: string, CKB_TEST_ADDRESS: string, testcases) {
  const casesLength = testcases.length;
  for (let i = 0; i < casesLength; i++) {
    const testcase = testcases[i];
    let loginResult;
    try {
      loginResult = await client.request('Login', testcase.payload);
    } catch (e) {
      if (testcase.error) {
    ...
    // }
    // if (testcase.send) {
    //   const ckbAddress = privateKeyToCkbAddress(ckb, CKB_PRI_KEY);
    //   assert(loginResult == ckbAddress);
    // }
  }
}
...
export async function rpcTestBTC(
  ...
  const { burnCases, txSummaryCases, balanceCases, feeCases, loginCases } = generateCases(
    CKB_TEST_ADDRESS,
    BTC_TEST_PUBKEY,
    ETH_TOKEN_ADDRESS,
  );
  await login(ckb, client, CKB_PRI_KEY, CKB_TEST_ADDRESS, loginCases);
  // await burn(ckb, client, CKB_PRI_KEY, CKB_TEST_ADDRESS, BTC_TEST_PUBKEY, burnCases);
